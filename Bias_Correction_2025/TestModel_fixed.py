...
model_path="checkpoints/CNNBC_2025_oldmodel/May-17-2025_1/Val-Loss_4.561.pth"
...
# Model instance - Using the fixed model
...
# IMPORTANT: Check if the model was trained with normalization
# If the model was trained WITHOUT normalization, don't normalize here
# If the model was trained WITH normalization, normalize here
data1=normalize(data1,date_string)  # Uncomment if model was trained with normalization
data1=torch.Tensor(data1).unsqueeze(dim=0)
...
# IMPORTANT: Apply the same normalization as during training
out_1=denormalize(out_1,date_string)  # Uncomment if model was trained with normalization

# Remove negative values (rainfall can't be negative)
out_1 = np.where(out_1 < 0, 0, out_1)
out_1 = np.where(np.isnan(mask1)==True, np.nan, out_1)

data1=np.array(data1)
data1=denormalize(data1,date_string)  # Uncomment if model was trained with normalization
...
print(f"Input data range: {np.nanmin(data1):.4f} to {np.nanmax(data1):.4f}")
print(f"Output data range: {np.nanmin(out_1):.4f} to {np.nanmax(out_1):.4f}")
print(f"Label data range: {np.nanmin(label1):.4f} to {np.nanmax(label1):.4f}")
