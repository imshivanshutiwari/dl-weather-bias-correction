...
gen_load_path = "/home/users/bipink/DIAT/George_Jose/Downscaling/GFS_4x/checkpoints/Make_Dataset/Set2/Jul-26-2025_1/Val-PSNR_48.673.pth"


#disc_load_path = "/home/hpcs_rnd/George_DIAT/Downscaling/srgan_pytorch/checkpoints/Make_Dataset/Set0/Mar-19-2025_adv5/DISC_Val-PSNR_42.708.pth"


print(f">> Loading GEN checkpoint from: {gen_load_path}")
#generator = Generator()
generator = Generator(in_channels=1)
generator.load_state_dict(torch.load(gen_load_path,weights_only=True))
...
todaysdate=date.today().strftime("%b-%d-%Y") + "_adv1"
# save path
...
# save parameters
#remark=""
#remark="Diff lrs and no scheduler"
...
for epoch in range(1, num_epochs + 1):
    ...
    for batch_idx, train_data in enumerate(tqdm(train_loader)):
        ...
        ## forward 
        train_gen_output = generator(train_lr_data)[:,:,1:-2,1:-2]
    ...
    if epoch % CONFIG["EVAL_EVERY"] == 0:
        ...
        with torch.inference_mode():
            ...
            for batch_ix, val_data in enumerate(tqdm(val_loader)):
                ...
                # Discriminator:
                ## forward
                val_gen_output = generator(val_lr_data)[:,:,1:-2,1:-2]
            ...
            if (Val_PSNR > best_psnr) and (epoch > CONFIG["DISC_LEAD"]):
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                torch.save(generator.module.state_dict(), save_path + f"GEN_Val-PSNR_{Val_PSNR:.3f}.pth")
                torch.save(discriminator.module.state_dict(), save_path + f"DISC_Val-PSNR_{Val_PSNR:.3f}.pth")
