...
class GFSdataset(Dataset):
    ...
    def __init__(self, data_path, label_path, idxs, scale_path=climatology_path):
        ...
        self.data=xr.open_dataset(data_path).rf.values[:,idxs,:,:]
        s1=self.data.shape
        self.data=self.data.reshape(s1[0]*s1[1],s1[2],s1[3])
        
        self.label=xr.open_dataset(label_path).rf.values[:,idxs,:,:]
        s2=self.label.shape
        self.label=self.label.reshape(s2[0]*s2[1],s2[2],s2[3])
        
        self.time=xr.open_dataset(label_path).time.values.astype('datetime64[h]')
        f = open(scale_path)
    ...
    def __getitem__(self, index):
        time1=self.time[index//(self.length//4880)]
