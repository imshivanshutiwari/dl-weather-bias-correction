...
#cnnbc = load_model("/home/hpcs_rnd/George_DIAT/Bias_Correction/CNNBC/models/my_cnntry9model.h5")
#cnnbc = load_model("/home/hpcs_rnd/George_DIAT/Bias_Correction/CNNBC/models/cnnbcmodel_4.h5")
cnnbc = load_model("/home/hpcs_rnd/George_DIAT/Bias_Correction/CNNBC/checkmodel1.keras")
...
maskstack2=np.repeat(mask1[np.newaxis, :, :], 1560, axis=0)
...
plt.pcolormesh(longitude[:32], latitude[:32],np.mean(data[24*52:],axis=0), cmap='brg', vmin = 0, vmax = level[1])
plt.colorbar()

plt.subplot(1,3,2)
plt.title('Bias corrected data')
#plt.contourf(results[index],cmap="rainbow", levels=a.levels, extend = "max")
plt.pcolormesh(longitude[:32], latitude[:32],np.mean(results[24*43:],axis=0), cmap='brg', vmin = 0, vmax = level[1])#, transform=ccrs.PlateCarree())
plt.colorbar()

plt.subplot(1,3,3)
plt.title('Observation data')
#plt.contourf(label[index],cmap="rainbow", levels=a.levels, extend = "max")
plt.pcolormesh(longitude[:32], latitude[:32],np.mean(label[24*43:],axis=0), cmap='brg', vmin = 0, vmax = level[1])
plt.colorbar()
